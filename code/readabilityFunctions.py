...
#Load easy word list
easy_words = open('../data/NDCeasywords.txt','r').read().replace('\n','').lower()
...
def analyze(data,textType='abstracttext',columnList={'doi','pmid','pubdate_year<>year'},tagger=tagger,doPreprocessing=1,ignoreSingleSentences=1):
    '''
    ...
    keepAbstract = qc.identify_badabstracts(data)
    if keepAbstract == 1: #This filters out any abstracts where the abstract is "no abstract" or includes "ABSTRACT TRUNCATED"
        if doPreprocessing==1:
            text = lang_prepropipeline(data,tagger)
        elif doPreprocessing==0:
            text = lang_minimalprepropipeline(data)
        ...
        if wc>0 and sc>0:
            ...
            lang['fre'] = fre
            lang['NDC'] = ndc[0]
            lang['PercDiffWord']=ndc[1]
            lang['DiffWord_lst']=ndc[2]
            lang['sylCount']=sylCount
            lang['wordLength']=wordLen
            lang['sentenceCount']=sc
            lang['wordCount']=wc
            lang['strippedText']=remainingText
        else:
            lang = None
    else:
         lang = None
