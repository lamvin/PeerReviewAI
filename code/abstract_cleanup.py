...
Modified by Philippe Vincent-Lamarre
"""
...
def cleanup_pretagger(text,punct):
...
def cleanup_sentenceEndsInNumber(text):
    ...
    if len(numberOccurs) > 0:
        ...
        for n in range(0,len(numberOccurs)):
            ...
            text=text[0:i] + text[i:i+reinfo.end()-3] + ' ' + text[i+reinfo.end()-3:]
            i=i+reinfo.end()
    return text
...
def cleanup_sentenceWithMissingSpaces(text):
    ...
    if len(numberOccurs) > 0:
        ...
        for n in range(0,len(numberOccurs)):
            reinfo=restr.search(text[i:])
            #Add a space when this occurs
            text=text[0:i] + text[i:i+reinfo.end()-1] + ' ' + text[i+reinfo.end()-1:]
            i=i+reinfo.end()
    return text
...
def cleanup_addSpaceAfterPeriod(text):
    ...
    if len(numberOccurs) > 0:
        ...
        for n in range(0,len(numberOccurs)):
            reinfo=restr.search(text[i:])
            #Add a space when this occurs
            text=text[0:i] + text[i:i+reinfo.start()+1] + ' ' + text[i+reinfo.end():]
            i=i+reinfo.end()
    return text
...
def cleanup_copyrightSentences(text):
    ...
    if reinfo!=None:
        text=text[0:reinfo.start()]
    restr=re.compile('This article is protected by copyright\. All rights reserved\.$')
    reinfo=restr.search(text)
    if reinfo!=None:
        text=text[0:reinfo.start()]
    return text
